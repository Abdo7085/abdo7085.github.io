...
(function() {
  ...
  function renderProductCard(product) {
    ...
    const techChips = techs.map(function(tech) {
      return '<span class="prod-card-tech">' + escapeHtml(tech) + '</span>';
    }).join('');
    ...
    return (
      '<a href="' + prefix + '/products/' + product.id + '.html" class="prod-card prod-fade-in product-link">' +
        '<div class="prod-card-img-wrapper">' +
          ...
          '<button type="button" class="prod-card-add" data-trigger="cart-add" data-product-id="' + escapeHtml(product.id) + '" aria-label="Add to cart" data-i18n-attr="aria-label:cart_aria_add_btn">' +
        ...
        '<div class="prod-card-content">' +
          ...
          '<div class="prod-card-footer">' +
            ...
            '<span class="prod-card-arrow">' + ARROW_RIGHT + '</span>' +
          '</div>' +
        '</div>' +
      '</a>'
    );
