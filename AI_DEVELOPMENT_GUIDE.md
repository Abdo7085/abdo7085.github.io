...
- **تحسين المظهر العام للمنتجات:** يجب استخدام أداة الذكاء الاصطناعي لاستخراج الخلفية `python scripts/remove_bg.py` لتصبح بصيغة (PNG شفافة) *(تأكد من تواجد `rembg[cpu]`)*، ثم `python scripts/crop_image.py` لقصها بإحكام لتندمج بطاقة المنتج شكلياً مع تصميم واجهة الموقع بسلاسة تامة.
